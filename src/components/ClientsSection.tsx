...
const ClientsSection = () => {
  ...
  return <section className="py-16 md:py-24 bg-[#f8f6f3]">
      <div className="container mx-auto px-[100px]">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8 mb-16">
          ...
          {/* Subtitle */}
          <p className="max-w-sm lg:text-center text-secondary">
            Confira quem impulsiona sua operação com a eficiência da Carflex.
          </p>

          {/* CTA Button */}
          <Button className="bg-accent hover:bg-accent/90 text-white px-8 py-6 text-sm font-semibold tracking-wide w-fit">
            CONHEÇA NOSSOS CASES
          </Button>
        </div>

        {/* Logos Row */}
        <div className="flex flex-wrap justify-center lg:justify-between items-center gap-8 md:gap-[30px]">
          {clients.map((client, index) => <div key={index} className="flex items-center justify-center h-16 md:h-20 hover:scale-105 transition-transform duration-300">
              <img src={client.logo} alt={client.name} className="h-full w-auto object-contain max-w-[140px] md:max-w-[180px]" />
            </div>)}
        </div>
      </div>
    </section>;
